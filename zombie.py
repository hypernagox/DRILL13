...
class Zombie:
    ...
    def compare_ball(self):
        boy_ball = play_mode.boy.ball_count
        return self.ball_count >= boy_ball
    def run_away(self,r = 0.5):
        boy_x ,boy_y = play_mode.boy.x,play_mode.boy.y
        self.dir = math.atan2(self.y - boy_y, self.x - boy_y)
        self.speed = RUN_SPEED_PPS
        ...
        if  not self.distance_less_than(self.tx,self.ty,self.x,self.y,r):
            return BehaviorTree.RUNNING
        else:
            return BehaviorTree.SUCCESS
    ...
    def is_boy_away(self, r = 0.5):
        return not self.distance_less_than(play_mode.boy.x, play_mode.boy.y, self.x, self.y, r)
    ...
    def build_behavior_tree(self):
        a1 = Action('Set target location',self.set_target_location,500,50) # 액션노드 생성
        ...
        SEQ_run_away = Sequence('거리가 7미터 이상 될 때까지 도망',c1,a6)
